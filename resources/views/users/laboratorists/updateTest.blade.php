...
<div class="container">
    ...
    <h1 style="text-align: center; font-size: 48px;padding-bottom: 1%;color:white;margin-top: 2%; background-color: rgb(0, 0, 0);">Update Lab Test</h1>
    ...
    @endif
    <form class="row g-3 needs-validation container" autocomplete="off"  style="padding:50px" action="{{route('updateTest')}}" method="post" novalidate>
        @csrf
        <input type="text" name="id" value="{{$data->id}}" hidden>
        <div class="col-md-6">
            <label for="test_name" class="form-label">Test Name</label>
            <input type="text" name="test_name" class="form-control" value="{{$data->name}}" id="test_name">
            <span class="text-danger">@error('test_name'){{ $message }}@enderror</span>
        </div>
        <div class="col-md-6">
            <label for="test_price " class="form-label">Test Price</label>
            <input type="text" name="test_price" value="{{$data->template}}" class="form-control" id="test_price">
